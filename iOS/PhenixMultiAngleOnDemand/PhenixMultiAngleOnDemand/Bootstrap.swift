...
final class Bootstrap {
        ...
        queue.sync(flags: [.barrier]) {
            ...
            do {
                ...
                DispatchQueue.main.async {
                    let viewController = self.makeInitialViewController(core: core, session: session)
    ...
    private func makeInitialViewController(core: PhenixCore, session: AppSession) -> UIViewController {
        let collectionViewModel = StreamCollectionViewController.ViewModel(core: core, session: session)
        let collectionDataSource = StreamCollectionViewController.DataSource(core: core)
        let collectionViewController = StreamCollectionViewController()
        collectionViewController.viewModel = collectionViewModel
        collectionViewController.dataSource = collectionDataSource

        let viewModel = StreamViewController.ViewModel(core: core, session: session)
        let viewController = StreamViewController.instantiate()
